<template>
	<scroll-view class="container" scroll-y="true">
			...
			<view class="take-photo">
				<view class="photo-icon" v-if="!photoSelected && !photoing">
					...
					<view class="paizhao-secion">
						...
						<view class="phone-text">请将摄像头按照对准口腔，确保拍摄角度与上图一致。</view>
					</view>
				</view>
		...
		<view class="analyse-section" v-if="photoTaken">
			...
			<view class="photo-text">口腔癌风险监测</view>
				...
				<view class="fangan-content">
					...
					<view class="fangan-list">
						<view class="fangan-item">
							<view class="dot dot1" :style="{backgroundColor:rick.text}"></view>
							<view class="text-section-1" :style="{color:rick.text,backgroundColor:rick.bac}">
								{{resData.problem}}
							</view>
						</view>
					</view>
					<view class="jiesaho">
						<view class="disease-title">可能症状</view>
						<image class="jieshao-img" src="../../static/cancer-zhengzhuang.jpg"></image>
						<view class="jieshao-text">口腔癌是发生在口腔的恶性肿瘤之总称，大部分属于鳞状上皮细胞癌，即所谓的黏膜发生变异。在临床实践中口腔癌包括牙龈癌、舌癌、软硬腭癌、颌骨癌、口底癌、口咽癌、涎腺癌、唇癌、和上颌窦癌以及发生于颜面部皮肤黏膜的癌症等。口腔癌是头颈部较常见的恶性肿瘤之一。</view>
					</view>
				</view>
			</view>
...
function takeAPhoto(){
	...
	uni.chooseImage({
	    ...
	    success: (res) => {
	        ...
	        // 选择成功后调用上传接口
	        uni.uploadFile({
	            url: baseUrl+"getpictures_cancer",
	            filePath: filePath,
	            name: 'file',
	            success: (uploadFileRes) => {
					...
					photoUrl.value=baseUrl+`shot_cancer/`+uploadpicname.value;
	            },
	            fail: (err) => {
...
console.log("确认图片")
	...
	uni.request({
		url: baseUrl+`confirm_shot_cancer/`+uploadpicname.value, // 替换为您的实际API地址
		method: 'GET',
		header: {
