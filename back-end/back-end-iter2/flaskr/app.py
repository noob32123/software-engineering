...
tongue_ip='http://192.168.43.234:5003/tongue'
...
def confirm_shot_tongue(picname):
    ...
    chat_comp = qianfan.ChatCompletion()
    sentence = "我的舌苔颜色为"+response.json()['coating_color']+"，我的舌头颜色为"+response.json()['tongue_color']+"，我的舌头的薄厚指标是"+response.json()['tongue_thickness']+"，我的舌头腐或腻的指标为"+response.json()['rotandgreasy']+"请分析一下我的口腔状况并提供一些中医和西医方面的口腔健康的建议，请按如下的json格式给我：{\"condition\":[{\"titile\":{},\"content\":{}}],\"analyse\":[{\"title\":{},\"content\":{}}]}只要单纯的json对象，不需要其它任何多余语言。"
    resp = chat_comp.do(model="ERNIE-Lite-8K-0308", messages=[{
        "role": "user",
        "content": sentence
    }])
